      ...
      <div className="w-full max-w-md space-y-8 rounded-xl bg-white p-8 shadow-sm dark:bg-zinc-900">
        ...
        {error && (
          <div className="rounded-lg bg-red-50 p-3 text-sm text-red-600 dark:bg-red-900/20 dark:text-red-400">
            ...
            {error === 'magic_link_failed' && 'Erro ao enviar link. Tente novamente.'}
            {error === 'google_auth_failed' && 'Erro na autenticacao Google.'}
            {error === 'auth_callback_error' && 'Erro na autenticacao. Tente novamente.'}
          </div>
        )}

        {message && (
          <div className="rounded-lg bg-green-50 p-3 text-sm text-green-600 dark:bg-green-900/20 dark:text-green-400">
            {message === 'check_email' && 'Cadastro realizado! Verifique seu email.'}
            {message === 'magic_link_sent' && 'Link enviado! Verifique seu email.'}
          </div>
        )}
